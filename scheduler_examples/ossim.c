...
void check_new_commands(queue_t *command_queue, queue_t *blocked_queue,
  ...
  do {
    ...
    if (client_fd < 0) {
      ...
      if (errno == ECONNABORTED)
        continue; // aborted handshake -> next
    ...
    // New PCBs do not have a time yet, will be set when we receive a RUN
    // message
  ...
  while (elem != NULL) {
    ...
    // Send ack message
    msg_t ack_msg = {.pid = current_pcb->pid,
 ...
 * pcb is moved to the command queue and an ACK message is sent back to the
 * client. If a client disconnects or an error occurs, the client is removed
 * from the blocked queue.
 *
 * @param blocked_queue The queue containing PCBs in I/O wait stated (blocked)
 * from CPU
 * @param command_queue The queue where PCBs ready for new instructions will be
 * moved
...
int main(int argc, char *argv[]) {
  ...
  // - COMMAND queue: for PCBs that are waiting for (new) instructions from the
  // app
  ...
  // We only have a single CPU that is a pointer to the actively running PCB on
  // the CPU
  pcb_t *CPU = NULL;
  ...
  while (1) {
    ...
    check_new_commands(&command_queue, &blocked_queue, &ready_queue, server_fd,
                       current_time_ms);
    ...
    // Tasks from the blocked queue could be moved to the command queue, check
    // again
